...
export const useChildrenList = () => {
  ...
  const loadChildren = async () => {
    try {
      ...
      if (!userInfo.userId) {
        console.warn('User ID not found in storage. This might happen if the API response does not include userId.');
        setChildren([]);
        return;
      }
      ...
      setError(err.response?.data?.message || err.message || 'Failed to load children');
    } finally {
      setLoading(false);
    }
  };
...
export const useChild = (childId: string | null) => {
  ...
  const loadChild = async (id: string) => {
    try {
      ...
      setError(err.response?.data?.message || err.message || 'Failed to load child details');
    } finally {
      setLoading(false);
    }
  };
