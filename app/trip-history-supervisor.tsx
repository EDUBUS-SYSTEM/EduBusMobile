...
import { getSupervisorTrips, getSupervisorTripsByDate } from "@/lib/supervisor/supervisor.api";
...
export default function SupervisorTripHistoryScreen() {
  ...
  const loadTrips = async (dateISO: string) => {
    ...
    try {
      ...
      const data = await getSupervisorTripsByDate(dateISO);
  ...
  const preloadMonth = async (year: number, month: number) => {
    ...
    try {
      for (let day = 1; day <= lastDay; day++) {
        ...
        try {
          const data = await getSupervisorTripsByDate(iso);
  ...
  const computeRangeAndTotal = async (semester: AcademicSemester | null) => {
    ...
    try {
      const dateFrom = `${start.getFullYear()}-${(start.getMonth() + 1).toString().padStart(2, "0")}-${start
        .getDate()
        .toString()
        .padStart(2, "0")}`;
      const dateTo = `${end.getFullYear()}-${(end.getMonth() + 1).toString().padStart(2, "0")}-${end
        .getDate()
        .toString()
        .padStart(2, "0")}`;
      const list = await getSupervisorTrips(dateFrom, dateTo);
      const pastOnly = list
        .map((t) => ({
          ...t,
          serviceDate: toDateOnly(t.serviceDate),
      ...
      // build cache by date
      const map: Record<string, DriverTripDto[]> = {};
      pastOnly.forEach((t) => {
        const key = toDateOnly(t.serviceDate);
        if (!map[key]) map[key] = [];
        map[key].push({
          id: t.id,
          routeId: "",
          ...
          status: t.status as DriverTripDto["status"],
          scheduleName: t.routeName,
      ...
      setTripCache(map);
      setTotalTrips(pastOnly.length);
    } catch (err) {
      console.warn("Failed to load supervisor trip range", err);
      setTotalTrips(0);
    } finally {
  ...
  const renderTripCard = (trip: DriverTripDto, index: number) => (
    <TouchableOpacity
      key={trip.id}
      activeOpacity={0.9}
      onPress={() => router.push(`/trip-history-supervisor/${trip.id}` as any)}
      style={{
        flexDirection: "row",
        marginBottom: 12,
      }}
